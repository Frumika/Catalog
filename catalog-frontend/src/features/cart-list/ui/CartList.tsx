...
//     const {cartPositions, loading, updateQuantity, removeItem, toggleCheck} = useCartStore();
//
//     if (loading && cartPositions.length === 0) return <div>Загрузка корзины...</div>;
//     if (cartPositions.length === 0) return <div>Корзина пуста</div>;
//
...
//
//     const activeItems = cartPositions.filter(item => item.checked);
//
